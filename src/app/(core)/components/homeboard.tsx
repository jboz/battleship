    ...
    <div className={styles.container}>
      ...
      <div className={styles.boardContainer}>
        ...
        {!started && (<div className={styles.ships}>
          {ships.filter(s => !s.placed).map((ship, index) => <BoardShip key={'ship' + index} ship={ship} onSelection={setSelectedShip} selected={selectedShip?.id === ship.id} />)}
        </div>)}
      </div>
    </div>
  )
...
const BoardSquare = ({ square, onClick, onMouseEnter, onMouseOut }: BoardSquareProps) => {
  return <div 
      ...
      onMouseOut={() => onMouseOut(square)}></div>
}
